...
class ClosedRequest(models.Model):
	...
	rating = models.IntegerField(verbose_name = "Качество обслуживания",null=True,blank = True)
	comment = models.CharField(max_length=200,  verbose_name='Комментарий исполнителя',blank = True)
	client_comment = models.CharField(max_length=200,  verbose_name='Комментарий клиента',blank = True)
	date = models.DateField(verbose_name = 'Дата')
	responsible = models.CharField(max_length=100, verbose_name = "Ответственный(е)",null=True,blank = True)
	is_accepted = models.BooleanField(default=False)

	def mark_photo_1(self):
		return mark_safe(' <a href = "%s" ><img style="height:300px; width: 300px;" src="%s" /></a>' %(self.photo_1.url,self.photo_1.url))
	mark_photo_1.short_description = "Фото 1"

	def mark_photo_2(self):
		return mark_safe(' <a href = "%s" ><img style="height:300px; width:300px;" src="%s" /></a>' %(self.photo_2.url,self.photo_2.url))
	mark_photo_2.short_description = "Фото 2"

	def mark_photo_3(self):
		return mark_safe(' <a href = "%s" ><img style="height:300px; width:300px;" src="%s" /></a>' %(self.photo_3.url,self.photo_3.url))
	mark_photo_3.short_description = "Фото 3"
